...
export const sOnsetI = new Array(
...
export const sConvertStandardise = new Array(
...
export const sAliWeb = new Array(
  'ص', 'ێ',
  'أ', 'ڕ'
);

export const sConvertDilan = new Array(
'ص', 'ڵ',
'ث', 'ێ',
'ذ', 'ڕ'
);
...
// Latin type Array
export const sConvertLatin = new Array(
  //typed, Hawar
  'ee', 'ê',
  'ii', 'î',
  'uu', 'û',
...
//Arabic type Array
export const sKeyboardLayouts = new Array(
...
export const convert = (
  ...
  switch (mode) {
      case 1: //Arabic to Latin
          ...
          if (options.chkDilan)
              for (let i = 0; i < sConvertDilan.length; i += 2)
                  s = s.replace(new RegExp(sConvertDilan[i], 'g'), sConvertDilan[i + 1]);
          if (options.chkAliWeb)
              for (let i = 0; i < sAliWeb.length; i += 2)
          ...
          if (options.chkCapitaliseSentence)
              s = s.replace(new RegExp('(^|\r\n|[\\.\\u003F!] [^a-zêîûçş0-9\'’]*)([a-zêîûçş])', 'gm'), function (m, p1, p2) {
                  for (let i = 0; i < sCapitalLetters.length; i += 2)
      ...
      case 2: //Latin to Arabic
          ...
          if (options.dchkConnectSuffixes)
              for (let i = 0; i < sSuffixes.length; i++)
      ...
      case 4: //Type Latin
          for (let i = 0; i < sConvertLatin.length; i += 2)
...
export const getKeyStroke = (e) => {
  ...
  for (let i = 0; i < sKeyboardLayouts.length; i += 4)
